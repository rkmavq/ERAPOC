...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <xsl:param name="IRBName"></xsl:param>
		...
		<div id="projectDetailDiv">
		   <table border="1" align="center" width="100%">
			   ...
			   <td>Sites:</td>
				<td>
					<xsl:choose>
						<xsl:when test="count(siteName) &gt; 0">
			   ...
			   <td>Funding Agency:</td>
			   	<td>
			   		<xsl:choose>
			   			<xsl:when test="count(fundingSourceComp) &gt; 0">
			   				<xsl:for-each select="fundingSourceComp">
			   					<xsl:choose>
	...
	<xsl:choose>
			...
			<xsl:choose>
			<xsl:when test="remType ='rem30'">
				The IRB requested that by <xsl:value-of select="remApprovalEndDate"/> you submit a completed Continuing/Final Review Progress Report and required attachments to request continuing approval of study closure.
			</xsl:when>
			<xsl:when test="remType = 'rem60'">

				<u>The PPHS submission deadline immediately prior to </u> <xsl:value-of select="remApprovalEndDate"/> or within 30 days of study closure, whichever is earlier, in order to request continuing IRB approval or study closure.
		...
		<xsl:choose>
			<xsl:when test="remType = 'rem30'">
			 If continuing review approval is not granted before the expiration date of <xsl:value-of select="$endDate"/>approval of this research expires on that date. If IRB approval expires, <u> all research activities must stop</u>. This includes recruitment, advertisement, screening, enrollment, consent, interventions, interactions, and collection of private identifiable information. Advertisements currently running in the media must be pulled.<br/><br/>
			</xsl:when>
			<xsl:when test="remType = 'rem60'">
		...
		<xsl:choose>
			<xsl:when test="remType = 'rem30'">
			 If you have any questions about how to submit this request, need help with the process, or have general questions about the IRB, please feel free to call us at 212-824-8200 or email us at IRB@mssm.edu.<br/><br/>
			</xsl:when>
			<xsl:when test="remType = 'rem60'">
		...
		If you have already submitted a Continuation/Final Report application in IDEATE, please disregard this reminder notice. <br/><br/>
		Sincerely,	<br/>
